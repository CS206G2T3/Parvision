...
const EQUIPMENT = [
  { id: 1, name: 'Driver TM-R15', location: 'Storage A', icon: IMG_SPORTS_GOLF },
  { id: 2, name: 'Iron Set 7-PW', location: 'Storage A', icon: IMG_SPORTS_GOLF },
  { id: 3, name: 'Golf Bag Pro', location: 'Storage B', icon: IMG_WAREHOUSE },
  { id: 4, name: 'Putter Scotty', location: 'Storage A', icon: IMG_SPORTS_GOLF },
  { id: 5, name: 'Wedge 56°', location: 'Storage C', icon: IMG_WAREHOUSE },
  { id: 6, name: 'Fairway Wood 3W', location: 'Storage B', icon: IMG_SPORTS_GOLF },
]

function EquipmentCard({ name, location, icon, selected, onToggle }) {
  return (
    <button
      onClick={onToggle}
...
export default function TakeOffPage() {
  ...
  const handleConfirm = () => {
    // TODO: call take-off API with selected IDs
    setConfirmed(true)
  ...
  return (
    <div className="relative w-full bg-white flex flex-col min-h-[852px]">
      ...
      <div className="relative w-full bg-white pt-14 px-6 pb-4 flex-shrink-0">
        ...
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-[#248a3d] rounded-full flex items-center justify-center flex-shrink-0">
            <img src={IMG_TAKE_OFF_PLAY} alt="Take off" className="w-[16px] h-[16px] object-contain" />
          </div>
          <h1
        ...
        <p
          ...
          style={{ fontFamily: '-apple-system, "SF Pro Text", system-ui, sans-serif' }}
        >
          Select the equipment you want to take out.
        </p>
      </div>
        ...
        {confirmed ? (
          ...
          >
            <span
              ...
              style={{ fontFamily: '-apple-system, "SF Pro Display", system-ui, sans-serif' }}
            >
              Confirm Take Off{selected.size > 0 ? ` (${selected.size})` : ''}
            </span>
          </button>
        )}
      </div>
